...
class App():
    ...
    def draw(self):
        ...
        #点数描画
        px.text(210, 40, f"SCORE:{self.score: >8}", px.COLOR_WHITE, self.BOF)   #点数描画
        px.text(210, 60, f"LINE :{self.linecnt: >8}", px.COLOR_WHITE, self.BOF) #消しライン数描画
        ...
        if self.gmovflg==1:
            ...
            px.text(70, 155, "GAME OVER", px.COLOR_WHITE, self.BOF)                #ゲームオーバー表示
            px.text(45, 165, "RETRY -> 左クリック ", px.COLOR_WHITE, self.BOF)
            px.text(115, 175, "or", px.COLOR_WHITE, self.BOF)
            px.text(100, 185, "ENTERキー", px.COLOR_WHITE, self.BOF)
