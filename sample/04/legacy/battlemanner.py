...
class BattleManner(BaseState):
    ...
    def draw(self):
        ...
        # ステータス表示
        for pos, pl in enumerate(self.pt):
            clr = px.COLOR_WHITE if pl.is_alive() else px.COLOR_RED
            act = pl.action if pl.action else ('？？', None)
            t = f"{Meth.pad(pl.name,4)} HP{Meth.pad(pl.hp,3)} {Meth.pad(act[0],4)}"
